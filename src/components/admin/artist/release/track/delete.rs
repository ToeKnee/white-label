...
pub fn DeleteTrack(track: Track) -> impl IntoView {
    ...
    view! {
        ...
        <dialog class="modal" node_ref=dialog_element>
            <div class="modal-box">
                <h3 class="text-lg font-bold">"Delete "{track.name.clone()}</h3>
                <p>"Are you sure you want to delete " {track.name.clone()} "?"</p>
                <p>"This action will be performed immediately."</p>
                <p>
                    "This will perform a soft delete. "{track.name}
                ...
                <div class="modal-action">
                    <ActionForm action=update_track>
                        ...
                        }} <input name="slug" type="hidden" value=track.slug />
